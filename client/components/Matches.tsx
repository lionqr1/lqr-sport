...
export default function Matches({ onWatch }: MatchesProps) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const [teamsRes, leaguesRes, matchesRes, matchSrcRes, chRes, tvRes] =
        await Promise.all([
  ...
  const vmatches = useMemo<MatchViewModel[]>(() => {
    ...
    const groupedSources = matchSources.reduce(
      (acc, src) => {
    ...
    return matches
      .filter((m: any) => {
        ...
        const unpub = m.unpublish_at
          ? new Date(m.unpublish_at).getTime()
          : null;
      ...
      .map((m) => {
        ...
        const sources = (groupedSources[m.id] || [])
          .map((src) => {
          ...
          <CardContent className="p-4 space-y-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <Button
                ...
                onClick={() => {
                  const title =
                    `${match.home.name} vs ${match.away.name} ${match.league ? `- ${match.league.name}` : ""}`.trim();
            ...
            {pickerOpen?.sources.map((s, idx) => (
              <Button
                ...
                onClick={() => {
                  ...
                  const ordered = [
                    s,
                    ...pickerOpen.sources.filter((x) => x.id !== s.id),
                  ];
                  const alts = ordered
                    .slice(1)
              ...
              >
                <span className="mr-2 inline-block px-2 py-0.5 rounded bg-gray-700 text-xs">
                  {idx + 1}
                </span>
                {s.label}{" "}
